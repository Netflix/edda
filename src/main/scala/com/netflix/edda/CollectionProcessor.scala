...
object CollectionProcessor extends StateMachine.LocalState[CollectionProcessorState] {
  ...
  case class DeltaResult(from: Actor, delta: Collection.Delta, origMeta: Map[String, Any] = Map())(
      ...
      def processDelta(d: Collection.Delta) = {
        ...
        d.added.foreach(rec => {
          if (logger.isInfoEnabled)
            logger.info(
              "{} Added {}/{};_pp;_at={}",
              Utils.toObjects(req, path, rec.id, rec.stime.getMillis)
            )
        })
        ...
        d.changed.foreach(update => {
          if (logDiffs.get.toBoolean) {
            lazy val diff: String =
              Utils.diffRecords(Array(update.newRecord, update.oldRecord), Some(1), path)
