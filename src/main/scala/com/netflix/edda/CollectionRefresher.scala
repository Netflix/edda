...
class CollectionRefresher(collection: Collection) extends Actor {
  ...
  lazy val cacheRefresh =
    Utils.getProperty("edda.collection", "cache.refresh", collection.name, "10000")

  // how often to purge history, default is every 6 hours
  lazy val purgeFrequency =
    Utils.getProperty("edda.collection", "purgeFrequency", collection.name, "21600000")
        ...
        Actor.self.loopWhile(keepLooping) {
          ...
          Actor.self.reactWithin(timeLeft(lastRun, timeout)) {
            ...
            case msg @ TIMEOUT => {
              ...
              if (amLeader) {
                val purge =
                  if (timeLeft(collection.lastPurge, purgeFrequency.get.toLong) > 0) false else true
                if (purge) {
            ...
            case msg @ Elector.ElectionResult(from, result) => {
              ...
              if (!amLeader && result) {
                ...
                val purgeJitter = (purgeFrequency.get.toLong * .2 * rand.nextDouble).toLong * (if (rand.nextBoolean)
                                                                                                 1
                                                                                               else
                                                                                                 -1);
  ...
  override def exceptionHandler = {
    case e: Exception =>
      if (logger.isErrorEnabled) logger.error(s"$collection failed to refresh", e)
  }
