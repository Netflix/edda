...
object ElasticSearchDatastore {
    ...
    obj.asScala.foreach(kv => {
      if (kv._1.contains('.')) {
        ...
        newObj
          ...
          .put(parts.tail.head, kv._2.getValue)
  ...
  def esToScala(obj: Any): Any = {
    ...
    obj match {
      ...
      case null                 => null
      case other =>
        throw new java.lang.RuntimeException("esToScala: don't know how to handle: " + other)
    }
  }
  ...
  protected def esFilterOp(key: String, value: Any, op: String): FilterBuilder = {
    ...
    op match {
      ...
      case "$in" =>
        value.asInstanceOf[Seq[Any]].head match {
      ...
      case "$regex" =>
        throw new java.lang.UnsupportedOperationException("$regex query not supported")
        ...
        if (value.size > 1) {
          FilterBuilders.andFilter(
            value
              ...
              .map(kv => esFilter(Map(key -> (kv._1, kv._2))))
              .toSeq: _*
          )
        } else {
  ...
  def esQuery(queryMap: Map[String, Any]): QueryBuilder = {
    if (queryMap.isEmpty) QueryBuilders.matchAllQuery
    else QueryBuilders.constantScoreQuery(esFilter(queryMap))
  }
  ...
  def initClient(name: String): Client = {
    this.synchronized {
      ...
      if (!clients.contains(cluster + "-" + addresses)) {
        ...
        clients += cluster + "-" + addresses -> addresses
          .split(',')
          .fold(new TransportClient(settings))(
            (client, addr) => {
              val parts = addr.asInstanceOf[String].split(':')
              client
                .asInstanceOf[TransportClient]
                .addTransportAddress(
                  new InetSocketTransportAddress(parts.head, parts.tail.head.toInt)
                )
            }
          )
          .asInstanceOf[Client]
      ...
      try {
        ixClient
          ...
          .addMapping(
            ...
            scala.io.Source
              .fromInputStream(
                getClass.getResourceAsStream("/elasticsearch/mappings/_default_.json")
              )
              .mkString
          )
          .execute
        ...
        case e: org.elasticsearch.indices.IndexAlreadyExistsException =>
          Unit // someone already beat us to it, ignore this
      }
    }
  }
}
...
class ElasticSearchDatastore(val name: String) extends Datastore {
  ...
  private lazy val monitorIndexName = Utils
    ...
    .get
    .replaceAll("[.]", "_")
  private lazy val retentionPolicy =
    Utils.getProperty("edda.collection", "retentionPolicy", name, "ALL")

  private val writeConsistencyProp =
    Utils.getProperty("edda", "elasticsearch.writeConsistency", name, "quorum")
  private def writeConsistency = WriteConsistencyLevel.fromString(writeConsistencyProp.get)
  private val replicationTypeProp =
    Utils.getProperty("edda", "elasticsearch.replicationType", name, "async")
  private def replicationType = ReplicationType.fromString(replicationTypeProp.get)

  private lazy val scanBatchSize =
    Utils.getProperty("edda", "elasticsearch.scanBatchSize", name, "1000");
  private lazy val scanCursorDuration =
    Utils.getProperty("edda", "elasticsearch.scanCursorDuration", name, "60000");
  private lazy val bulkBatchSize =
    Utils.getProperty("edda", "elasticsearch.bulkBatchSize", name, "0");
  ...
  def init() {
      ...
      if (nameParts.size == 2) "edda_1"
        ...
        Utils
          .getProperty(
            ...
            "elasticsearch.index",
            name,
            nameParts.take(nameParts.size - 2).mkString("_") + "_1"
          )
          .get
      }
    ...
    val mapping = scala.io.Source
      .fromInputStream(getClass.getResourceAsStream("/elasticsearch/mappings/default.json"))
      .mkString
    ixClient
      .preparePutMapping(indexName)
      .setType(docType)
      .setSource("{\"" + docType + "\": " + mapping + "}")
    ...
    if (!ixClient.prepareExists(aliasName).execute().actionGet().isExists()) {
      ixClient
        .prepareAliases()
        .addAlias(indexName, aliasName, FilterBuilders.typeFilter(docType))
        .execute
        .actionGet
    }
  ...
  def query(queryMap: Map[String, Any], limit: Int, keys: Set[String], replicaOk: Boolean)(
    ...
    val builder = if (idQuery) {
      val facet = FacetBuilders
        .termsFacet("f")
        .field("id")
        .size(1000000)
        .facetFilter(esFilter(query))
  ...
  def fetch(search: SearchRequestBuilder, limit: Int, keys: Set[String])(
    implicit req: RequestId
  ): Seq[Record] = {
    ...
    try {
      ...
      if (keys.size > 0)
        builder.addFields(
          (keys + "stime").map(s => s.split('.').take(2).mkString(".")).toSet.toSeq: _*
        )
      ...
      if (searchResp.getFailedShards() > 0) {
        ...
        failures.foreach(failure => {
          val ix = builder.request.indices.mkString(",")
          logger.error(
            s"$req [$ix] search shard failure on ${failure.shardId} reason: ${failure.reason} on query: $builder"
          )
        })
        throw new java.lang.RuntimeException(
          builder.request.indices.mkString(",") + " shard failures"
        )
      }
      if (idQuery) {
        ...
        searchResp
          ...
          .getEntries()
          .asScala
          .map(f => f.getTerm().toString())
          .toSet
          .map((id: String) => Record(id, null))
          .toSeq
      } else {
        searchResp
          .getHits()
            ...
            try esToRecord(if (keys.size > 0) esFieldsFixup(r.getFields) else r.getSource)
              ...
              case e: Exception => {
                if (logger.isErrorEnabled)
                  logger.error(s"$req$this failed to parse record: ${r.getSource}", e)
                throw e
              }
            }
          })
  ...
  def scan(search: SearchRequestBuilder, keys: Set[String] = Set())(
    implicit req: RequestId
  ): Seq[Record] = {
    import collection.JavaConverters.iterableAsScalaIterableConverter
    import org.elasticsearch.action.search.SearchResponse
      ...
      while (keepLooping) {
        ...
        if (scrollResp.isTimedOut) {
          ...
          throw new java.util.concurrent.TimeoutException(
            builder.request.indices
              .mkString(",") + " scanning query failed  after " + scanCursorDuration.get + "ms timeout"
          )
        }
        seq = seq ++ scrollResp
            .getHits()
  ...
  def collectionModified()(implicit req: RequestId): DateTime = {
    ...
    try {
      val response =
        client.prepareGet(monitorIndexName, "collection_mark", name).execute().actionGet()
  ...
  def markCollectionModified()(implicit req: RequestId) = {
    ...
    try {
      client
        .prepareIndex(monitorIndexName, "collection_mark")
        .setId(markRec.id)
  ...
  protected def upsert(record: Record)(implicit req: RequestId) {
    ...
    try {
      client
        .prepareIndex(writeAliasName, docType)
        .setId(record.toId())
  ...
  protected def upsert(records: Seq[Record])(implicit req: RequestId) {
    ...
    val t0 = System.nanoTime()
    try {
      val iter: Iterator[Seq[Record]] =
        if (bulkBatchSize.get.toInt > 0) records.sliding(bulkBatchSize.get.toInt)
        else List(records).iterator
      iter.foreach(recs => {
  ...
  protected def remove(record: Record)(implicit req: RequestId) {
    ...
    try {
      val response = client
        .prepareDelete(writeAliasName, docType, record.toId())
        .setRouting(record.id)
        .execute()
        .actionGet();
  ...
  override def remove(queryMap: Map[String, Any])(implicit req: RequestId) {
    ...
    try {
      val response = client
        .prepareDeleteByQuery(writeAliasName)
        .setTypes(docType)
    ...
    try {
      ...
      iter.foreach(recs => {
        ...
        recs.foreach(rec => {
          bulk.add(
            client
              .prepareDelete(writeAliasName, docType, rec.id + "|" + rec.stime.getMillis)
              .setRouting(rec.id)
          )
        })
        val response = bulk.execute.actionGet
        if (response.hasFailures()) {
          if (logger.isErrorEnabled)
            logger.error(s"$req$this failed to bulk delete: ${response.buildFailureMessage()}")
        }
      })
    } catch {
      case e: Exception => {
