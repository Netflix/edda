...
abstract class Collection(val ctx: Collection.Context) extends Queryable {
  ...
  lazy val ignoreHistoryUpdateFailures =
    Utils.getProperty("edda.collection", "ignoreHistoryUpdateFailures", name, "false")
  ...
  private[this] val updatedRecordGauge = new BasicGauge[java.lang.Long](
    ...
    new Callable[java.lang.Long] {
      ...
      def call() =
        if (elector.isLeader()(RequestId("crawl.updatedRecordCount.gauge"))) updatedRecordCount.get
        else 0
    }
  )
  ...
  private[this] val addedRecordGauge = new BasicGauge[java.lang.Long](
    ...
    new Callable[java.lang.Long] {
      ...
      def call() =
        if (elector.isLeader()(RequestId("crawl.addedRecordCount.gauge"))) addedRecordCount.get
        else 0
    }
  )
  ...
  private[this] val deletedRecordGauge = new BasicGauge[java.lang.Long](
    ...
    new Callable[java.lang.Long] {
      ...
      def call() =
        if (elector.isLeader()(RequestId("crawl.deletedRecordCount.gauge"))) deletedRecordCount.get
        else 0
    }
  )
  ...
  protected[edda] override def initState =
    addInitialState(
      super.initState,
  ...
  protected override def init() {
    ...
    Utils.namedActor(this + " init") {
      ...
      if (Utils.getProperty("edda.collection", "jitter.enabled", name, "true").get.toBoolean) {
        val cacheRefresh =
          Utils.getProperty("edda.collection", "cache.refresh", name, "10000").get.toLong
        ...
        if (dataStore.isDefined) {
          ...
          purgePolicy match {
            ...
            case PurgePolicy.LAST => {
              if (logger.isWarnEnabled)
                logger.warn(s"$req$this LAST PurgePolicy is not yet implelemented")
            }
...
/** for setting the name on Collections when we are tracking many accounts with same root name.  Used with MergedCollection
  ...
  */
abstract class RootCollection(val rootName: String, accountName: String, ctx: Collection.Context)
